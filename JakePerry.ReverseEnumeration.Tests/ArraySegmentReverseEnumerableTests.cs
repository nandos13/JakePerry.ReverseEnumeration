...
{
    [TestClass]
    public class ArraySegmentReverseEnumerableTests
    {
        [TestMethod("Empty enumerable")]
        public void TestMethod_EmptyDoesNotEnumerate()
        {
            var reverseEnumerable = new ArraySegmentReverseEnumerable<string>(new ArraySegment<string>());
        ...
        {
            var enumerator = new ArraySegmentReverseEnumerator<string>(new ArraySegment<string>());
        ...
        {
            var enumerator = new ArraySegmentReverseEnumerator<string>(new ArraySegment<string>(GetAbcArray()));
        ...
        public void TestMethod_ForeachOrderReversed()
        {
            ...
            var reverseEnumerable = new ArraySegmentReverseEnumerable<string>(new ArraySegment<string>(sourceArray));
        ...
        {
            var source = new ArraySegment<string>(GetAbcArray());
            var reverseEnumerable = new ArraySegmentReverseEnumerable<string>(source);

            Assert.AreEqual(source.Count, reverseEnumerable.Count);

            for (int i = 0; i < source.Count; i++)
                Assert.AreEqual(source.Array[i], reverseEnumerable[source.Count - 1 - i]);
        }
